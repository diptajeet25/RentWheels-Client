...
queryFn:async()=>{
const res=await fetch("https://car-rental-server-six-gold.vercel.app/dashboard/car-stats")
...
return(
<div className="px-6 py-6 text-base-content">
<div className="mb-12">
...
<p className="text-sm text-base-content/60">Total Cars</p>
<p className="text-3xl font-semibold mt-2">{data?.totalCars}</p>
</div>
<div className="border rounded-xl p-6 bg-base-100">
<p className="text-sm text-base-content/60">Booked Cars</p>
<p className="text-3xl font-semibold mt-2">{data?.bookedCars}</p>
</div>
<div className="border rounded-xl p-6 bg-base-100">
<p className="text-sm text-base-content/60">Available Cars</p>
...
<ResponsiveContainer>
<PieChart>
<Pie data={carsData} dataKey="count" nameKey="category" cx="50%" cy="50%" outerRadius={110} label>
