...
queryFn:async()=>{
const res=await fetch(`https://car-rental-server-six-gold.vercel.app/allcars?search=${search}&category=${category}&status=${status}&sort=${sort}&page=${page}&limit=${limit}`)
return res.json()
}
})
...
<div>
<Navbar/>
<h2 className="text-3xl lg:text-4xl font-bold text-center my-10 text-base-content">
Explore <span className="text-primary">Cars</span>
</h2>
<div className="w-[90%] mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
<input type="text" placeholder="Search by car name" className="input input-bordered" value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}}/>
...
<p className="text-base-content/60">No cars found</p>
</div>
)}
<div className="flex justify-center gap-3 my-10">
{[...Array(totalPages).keys()].map(num=>(
<button key={num} onClick={()=>setPage(num+1)} className={`btn ${page===num+1?'btn-primary':''}`}>
{num+1}
</button>
))}
</div>
